...
const App = () => {
  if (!window.localStorage.currentUser) {
    // persists the current user across page refresh
    window.localStorage.setItem("currentUser", Date.now().toString()); // if no current user, set userID to "testUser"
  }

  if (!window.localStorage.timestamp) {
    // persists the timestamp across refresh
    window.localStorage.setItem("timestamp", Date.now().toString()); // if no timestamp, sets it to now
  }
  ...
  window.Appcues.identify(userId, {
    // feel free to add in any user properties here
    role: "tester",
  ...
  window.Appcues.loadLaunchpad("#launchpad", {
    // loads Launchpad on this element
    position: "right",
  });
